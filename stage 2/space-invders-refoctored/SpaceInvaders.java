...
public class SpaceInvaders extends JFrame implements Commons {
	...
	private static final String INITIAL_MESSAGE = "Help us, awesome captain!!"
			+ "<br>The aliens are trying to invade our planet."
			+ "<br><br><br>Your mission:"
			+ "<br><br>Kill all the invading aliens before they manage to invade planet Earth."
	...
	private GameFacade gameFacade; // New: Composes facade

	public SpaceInvaders() {
		gameFacade = new GameFacade(); // Initialize facade
	...
	private class ButtonListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent event) {
			...
			gameFacade.startGame(); // Use facade to start
			frame.getContentPane().add(gameFacade.getGamePanel()); // Add via facade
			frame.setResizable(false);
			frame.setLocationRelativeTo(null);

			// Add key listener to frame, delegate to facade
			frame.addKeyListener(new KeyAdapter() {
				@Override
				public void keyPressed(KeyEvent e) {
					gameFacade.handleKeyPressed(e);
				}

				@Override
				public void keyReleased(KeyEvent e) {
					gameFacade.handleKeyReleased(e);
				}
			});
			frame.setFocusable(true); // Ensure frame can receive keys
